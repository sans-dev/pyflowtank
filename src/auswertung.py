...
def play_background_subtraction(path):
    ...
    while (cap.isOpened()):
        ...
        start = int(width/3)
        x_coords = list(range(width-start,width))
        y_coords = list(range(height))
        if not np.mod(frame_counter,10):
            x_coords = np.array(x_coords[::10])
